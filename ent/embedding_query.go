...
func (eq *EmbeddingQuery) QueryPrescription() *PrescriptionQuery {
	query := (&PrescriptionClient{config: eq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, false, embedding.PrescriptionTable, embedding.PrescriptionColumn),
		)
...
func (eq *EmbeddingQuery) FirstID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (eq *EmbeddingQuery) FirstIDX(ctx context.Context) uuid.UUID {
...
func (eq *EmbeddingQuery) OnlyID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (eq *EmbeddingQuery) OnlyIDX(ctx context.Context) uuid.UUID {
...
func (eq *EmbeddingQuery) IDs(ctx context.Context) (ids []uuid.UUID, err error) {
...
func (eq *EmbeddingQuery) IDsX(ctx context.Context) []uuid.UUID {
	ids, err := eq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (eq *EmbeddingQuery) Clone() *EmbeddingQuery {
	...
	return &EmbeddingQuery{
		...
		withPrescription: eq.withPrescription.Clone(),
...
func (eq *EmbeddingQuery) WithPrescription(opts ...func(*PrescriptionQuery)) *EmbeddingQuery {
	query := (&PrescriptionClient{config: eq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	eq.withPrescription = query
	return eq
}
...
func (eq *EmbeddingQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Embedding, error) {
	var (
		...
		_spec       = eq.querySpec()
		loadedTypes = [1]bool{
			eq.withPrescription != nil,
		}
	)
	if eq.withPrescription != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := eq.withPrescription; query != nil {
		if err := eq.loadPrescription(ctx, query, nodes, nil,
			func(n *Embedding, e *Prescription) { n.Edges.Prescription = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (eq *EmbeddingQuery) loadPrescription(ctx context.Context, query *PrescriptionQuery, nodes []*Embedding, init func(*Embedding), assign func(*Embedding, *Prescription)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Embedding)
	for i := range nodes {
		if nodes[i].embedding_prescription == nil {
			continue
		}
		fk := *nodes[i].embedding_prescription
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "embedding_prescription" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (eq *EmbeddingQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(embedding.Table, embedding.Columns, sqlgraph.NewFieldSpec(embedding.FieldID, field.TypeUUID))
	_spec.From = eq.sql
